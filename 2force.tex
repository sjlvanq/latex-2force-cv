...
\newcommand{\tagP}[1]{\cvtag{ColorTagPrimary}{#1}}
\newcommand{\tagS}[1]{\cvtag{ColorTagSecondary}{#1}}
...
\newcommand{\degree}[3]{%
  ...
  \textbf{#2} \hfill \textcolor{ColorMuted}{#1}\\
  \hspace*{1em}#3\par\vspace{4pt}}}
...
\newcommand{\job}[3]{%
  {\fontsize{10}{12}\selectfont\bfseries\color{ColorMainText}#1}\par
  {\fontsize{8.5}{11}\selectfont\color{ColorMuted}#2}\par\vspace{4pt}
...
\newcommand{\jobnote}[4]{%
  {\fontsize{10}{12}\selectfont\bfseries\color{ColorMainText}#1}\par
  {\fontsize{8.5}{11}\selectfont\color{ColorMuted}#2}\par\vspace{2pt}
  {\fontsize{9.5}{13}\selectfont\color{ColorMainText}#3}\par\vspace{#4}}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=north west, text width=55mm, inner sep=0pt] at ([xshift=7mm, yshift=-15mm]current page.north west) {
        ...
        {\fontsize{7}{11}\selectfont\linespread{1.2}\selectfont\raggedright
          \tagS{React} \tagS{Node.js} \tagS{AWS} \tagS{Docker}\\[3pt]
          \tagS{PostgreSQL} \tagS{MongoDB} \tagS{Git/GitHub} \tagS{REST APIs}\\[3pt]
          \tagS{Webpack} \tagS{Jest/Testing} \tagS{CI/CD}\par 
        }
...
\project%
  ...
  {\tagP{React}\tagP{Node.js}\tagP{PostgreSQL}}%
  {%
...
\project%
  ...
  {\tagP{React}\tagP{TypeScript}\tagP{MongoDB}}%
  {%
...
\project%
  ...
  {\tagP{React}\tagP{D3.js}\tagP{TypeScript}}%
  {%
...
\job{Senior Full-Stack Software Engineer}
    {Cloudwave Technologies Inc. / 2020--Present}
    {
...
\degree{2019}{Bachelor of Science in Computer Science}{University of Washington, Seattle}
